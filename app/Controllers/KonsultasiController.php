...
class KonsultasiController extends BaseController
{
    ...
    public function processVerifikasi()
    {
        ...
        if ($action == 'reject') {
            ...
            // Opsional: Email ke Klien kalau ditolak
            // (Anda bisa tambahkan send_email_brevo disini jika mau)

            session()->setFlashdata('msg', 'Pengajuan telah ditolak.');
        } else {
            // Approve / Reschedule (Set Tanggal & Lawyer)
            $no_bas = $this->request->getPost('no_bas');
            $tgl_baru = $this->request->getPost('tanggal_fiksasi');
            $lokasi = $this->request->getPost('meeting');

            $this->konsultasiModel->update($id_konsultasi, [
                'no_bas' => $no_bas,
                'meeting' => $lokasi,
                'tanggal_fiksasi' => $tgl_baru,
                'status' => 'waiting_lawyer' // Status kembali menunggu konfirmasi lawyer
            ]);

            $userModel = new UserModel();
            
            // 1. [EMAIL KE LAWYER] (Ada Tugas Baru / Jadwal Baru)
            $lawyer = $userModel->where('no_bas', $no_bas)->first();
            if ($lawyer) {
                $tglFormat = date('d M Y H:i', strtotime($tgl_baru));
                $subject = "[TUGAS] Konfirmasi Jadwal Konsultasi";
                $msg = "
                    <h3>Halo, {$lawyer['nama']}</h3>
                    <p>Sekretaris telah menetapkan jadwal konsultasi (Baru/Reschedule).</p>
                    <p><strong>Jadwal Usulan:</strong> $tglFormat WIB</p>
            ...
            // 2. [EMAIL KE KLIEN] (Info Jadwal Sedang Diajukan) --> INI YANG BARU
            // Ambil data klien dulu
            $klienData = $this->konsultasiModel->select('user.email, user.nama')
                ->join('user', 'user.id_user = konsultasi.id_user')
                ->where('konsultasi.id_konsultasi', $id_konsultasi)
                ->first();
            ...
            if ($klienData) {
                $tglFormat = date('d M Y H:i', strtotime($tgl_baru));
                $subjectKlien = "[UPDATE] Jadwal Konsultasi Sedang Diproses";
                $msgKlien = "
                    <h3>Halo, {$klienData['nama']}</h3>
                    <p>Admin/Sekretaris kami telah mengajukan jadwal (baru) untuk konsultasi Anda.</p>
                    <ul>
                        <li><strong>Tanggal Usulan:</strong> $tglFormat WIB</li>
                        <li><strong>Lokasi/Link:</strong> $lokasi</li>
                    </ul>
                    <p>Status saat ini: <strong>Menunggu Konfirmasi Lawyer</strong>.</p>
                    <p>Anda akan menerima email lagi segera setelah Lawyer menyetujui jadwal ini.</p>
                ";
                send_email_brevo($klienData['email'], $klienData['nama'], $subjectKlien, $msgKlien);
            }

            session()->setFlashdata('success', 'Jadwal diperbarui. Lawyer & Klien telah dinotifikasi via Email.');
        }
